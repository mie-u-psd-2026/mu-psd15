...
| 観点 | 内容 |
|---|---|
| **Where**(どこで動くか) | ブラウザ: Chrome / Safari の最新版。サーバー: Python 3.11〜3.13 + Flask、ポート **5001**(macOS では 5000 を AirPlay が使うため)。AI: Ollama、ポート 11434。3つとも同じパソコンの中 |
| **Where**(ネット接続) | **不要**。AIも Vue 本体も手元に置く。データベースも外部サービスも使わない |
| **Who**(誰が使うか) | 店員役の練習者1人。ログインも権限の区別もない。同時に複数人が使う想定はしない |
| **When**(いつ動くか) | 利用者が「練習を始める」「送信」「やめる」「もう一度」を押したとき。定期実行やバッチはない。サーバー起動時にだけ AI モデルの事前読み込みを1回行う |
| **How**(通信の決まり) | HTTP、RESTful API、JSON、文字コード UTF-8。エラーは RFC 7807(Problem Details)形式 |
| **How**(主要ライブラリ) | Flask 3.1、openai 2.46(Ollama の OpenAI 互換 API を呼ぶため)、pydantic 2.13、Vue.js 3(手元に配置)、pytest 9.1(自動テスト)。正確な版は `requirements.txt` |
| **How**(認証・セキュリティ) | 認証なし。API キーは Ollama 向けのダミー文字列。CORS 設定は不要(画面と API を同じサーバーが配る) |
...
**練習を何度もくり返す道具**である以上、見た目の緊張感より、長く使える読みやすさを優先した。熱の演出そのものは残し、地の色だけを反転させている。
